...
**Full skill**: [[aops-core/skills/aops/SKILL.md]] - invoke `activate_skill(name='aops')` for component patterns, workflows, compliance refactoring.
...
```
activate_skill(name="critic", model="opus", prompt="Review this for errors and hidden assumptions: [SUMMARY]")
```
...
4. **If NO skill exists**: Create one first via `activate_skill(name='framework')`
