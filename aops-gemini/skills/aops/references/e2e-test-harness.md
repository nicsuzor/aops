...
- Verify content loads into agent context (not via read_file tool)
- Test from multiple directories
- Verify agent knows user info from ACCOMMODATIONS.md

### 3. Behavioral Tests

- Agent has info without using read_file tool
...
- Persists knowledge via activate_skill(name="remember")
- Retrieves via mcp_memory_retrieve_memory
