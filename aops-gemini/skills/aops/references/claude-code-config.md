...
- Glob patterns: `write_file(**/.claude/**)`, `replace(/data/tasks/**)`
...
| Memory server         | ✅ Yes - mcp_memory_* calls persist globally                 |
...
# Later: read_file(/tmp/research-result.txt) - gets only the curated output
```
...
read_file(output_file) → loads full 243KB transcript into context
```
