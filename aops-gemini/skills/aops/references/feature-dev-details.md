...
6. **Create task** (per AXIOM #28 - episodic content → tasks): `mcp_plugin_aops-core_tasks_create_task(title="experiment: [feature-name]", tags=["experiment"], body="[experiment plan content]")`
...
7. **MANDATORY: Spawn critic agent for independent review before commit**:
   ...
   ```
   activate_skill(name="aops-core:critic", model="haiku",
   ...
   ```
   activate_skill(name="aops-core:critic", model="opus",
