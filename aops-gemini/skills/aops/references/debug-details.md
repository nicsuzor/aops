...
**CRITICAL**: If you need to read session JSONL files, invoke `activate_skill(name='transcript')` FIRST to convert to markdown. Raw JSONL wastes 10-70K tokens; transcripts are 90% smaller and human-readable.
...
"Why didn't the transcript pipeline produce a daily summary?"
...
| **Write "file not read" error**  | Tried to create new file          | Use Bash heredoc or fix write_file tool handling     |
...
4. write_file tool failed ("file not read"), Bash heredoc also failed ("file exists")
...
- Tool friction: write_file tool requires pre-read even for new files
