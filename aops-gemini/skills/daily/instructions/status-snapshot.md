...
Report counts only. Do not annotate with "→ recommended tasks" pointers.
...
Pull tasks with `due` ≤ 7 days via `mcp_pkb_list_tasks(format=json)` and sort by due date ascending. List each on its own line:
