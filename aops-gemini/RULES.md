...
DO NOT begin a third or fourth Edit/write_file tool call across multiple files in this session without a prior approved plan visible in the transcript — an `EnterPlanMode` block or the user's explicit "go ahead" on a stated plan. Single-file fixes, doc updates, and surgical bug fixes do not trigger.
